...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50">
      ...
      <header className="sticky top-0 bg-white border-b border-orange-100 shadow-sm z-10">
        <div className="max-w-2xl mx-auto px-4 py-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Croissant</h1>
          </div>
          <p className="text-sm text-gray-600">Encuentra tu sucursal de distribución</p>
        </div>
      </header>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 text-lg border-2 border-orange-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition"
            />
          </div>
              ...
              <div>
                {busqueda && (
                  <p className="text-sm text-gray-600">
                    <span className="font-semibold text-gray-900">{filtrados.length}</span> resultado{filtrados.length !== 1 ? 's' : ''} de{' '}
                    <span className="font-semibold text-gray-900">{sucursales.length}</span> sucursales
                  </p>
                )}
                {!busqueda && (
                  <p className="text-sm text-gray-600">
                    Total de <span className="font-semibold text-gray-900">{sucursales.length}</span> sucursales
                  </p>
                )}
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs text-orange-100">Sucursal</p>
                        ...
                        <div className="text-right">
                          {sucursal.nombre_sucursal && (
                            <p className="text-sm font-semibold text-white bg-orange-600 bg-opacity-50 px-3 py-1 rounded-full">
                              {sucursal.nombre_sucursal}
                            </p>
                          )}
                          <p className="text-xs text-orange-100 mt-2">{sucursal.marca}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 space-y-3">
                      {/* Razón Social */}
                      <div>
                        <p className="text-xs text-gray-500 font-medium mb-1">Razón Social</p>
                      ...
                      {sucursal.geolocalizacion_url ? (
                        <a
                          ...
                          className="mt-4 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold py-3 rounded-lg transition-all active:scale-95 touch-manipulation"
                        >
                          <Navigation className="w-5 h-5" />
                          Ver en Google Maps
                        </a>
                      ) : (
                        <div className="mt-4 w-full py-3 bg-gray-100 text-gray-500 rounded-lg text-center text-sm font-medium">
                          Maps no disponible
                        </div>
                      )}
                    </div>
      ...
      {/* Footer */}
      <footer className="mt-12 py-6 border-t border-orange-100 text-center text-xs text-gray-600">
